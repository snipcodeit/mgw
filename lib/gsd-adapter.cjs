...
/**
 * Safe import of model-fallback.cjs — may not exist if #233 not merged yet.
 * @type {typeof import('./model-fallback.cjs').ModelFallbackEngine|null}
 */
...
try {
  ...
  // model-fallback.cjs not available — fallback features disabled
}

/** @type {import('./model-fallback.cjs').ModelFallbackEngine|null} */
let _fallbackEngineSingleton = null;

/**
 * Get or create the singleton ModelFallbackEngine instance.
 *
 * The engine loads configuration from .mgw/config.json on first call.
 * Returns null if model-fallback.cjs is not available.
 *
 * @returns {import('./model-fallback.cjs').ModelFallbackEngine|null}
 */
...
/**
 * Resolve the ordered model fallback chain for a GSD agent type.
 *
 * When model fallback is available and enabled, returns the full chain
 * (e.g., ['inherit', 'sonnet', 'haiku'] for gsd-planner).
 *
 * When model fallback is not available or disabled, returns a single-element
 * array with the primary model resolved via gsd-tools resolve-model.
 *
 * @param {string} agentType - GSD agent type (e.g. 'gsd-planner', 'gsd-executor')
...
function resolveFallbackChain(agentType) {
  ...
  // Fallback not available or disabled — return primary model only
  try {
