...
/**
 * Detect the mgw:project STATE_CLASS from five signals.
 *
 * Encapsulates the classification logic from workflows/detect-state.md so that
 * it is testable in JavaScript without spawning shell processes.
 *
 * Signals:
 ...
 *   G — Count of GitHub milestones (caller is responsible for fetching this)
 *
 * State classes:
 *   Fresh        — no signals active
 *   GSD-Only     — P=true, M=false, G=0
 *   GSD-Mid-Exec — P=true, R=true (or S=true), M=false, G=0
 *   Aligned      — M=true, G>0, local milestone count consistent with G
 *   Diverged     — M=true, G>0, local milestone count inconsistent with G
 *   Extend       — M=true, G>0, all milestones complete (current_milestone > count)
 *
 * Consistency check simplification:
 *   The bash version calls `gh api` a second time to do a name-overlap check.
 *   This JS version uses only count proximity (|local - G| <= 1) to keep the
 *   function pure and testable. Callers that need name-overlap validation should
 *   implement it outside this function.
 *
 * @param {object} [options={}]
 * @param {string} [options.repoRoot] - Absolute path to repo root. Defaults to process.cwd().
 * @param {number} [options.githubMilestoneCount=0] - Count of open GitHub milestones (G signal).
 * @returns {{ stateClass: string, signals: { P: boolean, R: boolean, S: boolean, M: boolean, G: number } }}
 */
function detectProjectState(options = {}) {
  ...
  // Edge case: M=true but G=0, or M=false but G>0 → treat as Fresh
  return { stateClass: 'Fresh', signals };
}
