 ...
 * Returns ~/.opencode/commands/ -- commands are user-installed, not bundled with mgw.
 * @returns {string} Absolute path to ~/.opencode/commands/
 * @throws {Error} If commands/ directory does not exist
 */
function getCommandsDir() {
  const dir = path.join(os.homedir(), '.opencode', 'commands');
...
function invoke(commandFile, userPrompt, opts) {
  ...
  return new Promise(function(resolve, reject) {
    var stdio = o.quiet ? ['pipe', 'pipe', 'pipe'] : ['inherit', 'inherit', 'inherit'];

    var child = spawn('opencode', args, { stdio: stdio });
