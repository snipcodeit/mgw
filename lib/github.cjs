 ...
 * Critical-path functions use withRetry for transient failure recovery.
 * Non-blocking functions (getProjectNodeId, findExistingBoard, etc.)
 * handle errors internally and return null on failure.
 */

const { execSync } = require('child_process');
const { TimeoutError, GitHubApiError } = require('./errors.cjs');
...
/**
 * Parse an HTTP status code from gh CLI stderr output.
 * @param {string} stderr
 * @returns {number|null}
 */
...
/**
 * Execute a shell command synchronously and return its stdout as a trimmed string.
 * Throws enriched errors for classification by retry infrastructure.
 * @param {string} cmd
 * @returns {string}
 * @throws {TimeoutError} If command exceeds GH_TIMEOUT_MS
 * @throws {GitHubApiError} If command fails
 */
function run(cmd) {
  try {
    return execSync(cmd, {
    ...
    if (err.killed) {
      throw new TimeoutError(
        `gh command timed out after ${GH_TIMEOUT_MS / 1000}s: ${cmd.slice(0, 80)}`,
    ...
    // Preserve Node.js error code for classifyFailure network detection
 ...
 * @returns {Promise<object>} Parsed issue JSON
 */
 ...
 * @returns {Promise<Array<object>>} Parsed array of issue objects
 */
 ...
 * @returns {Promise<object>} Parsed milestone JSON
 */
 ...
 * @returns {Promise<{remaining: number, limit: number, reset: number}>} Core rate limit
 */
 ...
 * @param {number|string} number - Milestone number
 * @returns {Promise<object>} Parsed updated milestone JSON
 */
 ...
 * @returns {Promise<string>} Output from gh release create
 */
...
/**
 * Resolve the GraphQL node ID for an existing GitHub Projects v2 board by number.
 * Tries user projects first, falls back to org projects.
 * Non-blocking: returns null if the API call fails.
 * @param {string} owner - GitHub user or org login
 * @param {number} projectNumber - Project board number
 * @returns {Promise<string|null>} node ID (e.g. "PVT_...") or null on failure
 */
 ...
 * @returns {Promise<{ number: number, url: string, nodeId: string, title: string } | null>}
 */
 ...
 * @param {string} owner - GitHub user or org login
 * @param {number} projectNumber - Project board number
 * @returns {Promise<object|null>} Fields object keyed by: status, ai_agent_state, phase, gsd_route, milestone
 */
 ...
 * @returns {Promise<{number: number, url: string}>} Project number and URL
 */
 ...
 * @returns {Promise<{ posted: boolean, method: 'discussion'|'comment'|'none', url: string|null }>}
 */
