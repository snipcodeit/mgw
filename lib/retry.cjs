 ...
 */
const _PERMANENT_STATUS_CODES = new Set([400, 401, 404, 422]);
 ...
 *
 * Calls `fn()`, and on failure:
 *   1. Classifies the error via `classifyFailure()`
 *   2. If transient and retries remain, waits `getBackoffMs()` then retries
 *   3. If permanent or retries exhausted, throws the last error
 *
 * @param {() => Promise<*>} fn - Async function to execute
 * @param {object} [opts]
 * @param {number} [opts.maxRetries] - Override MAX_RETRIES (default: 3)
 * @returns {Promise<*>} Result of fn()
 * @throws {Error} Last error if all retries exhausted or error is permanent
 */
