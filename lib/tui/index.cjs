...
async function createIssuesBrowser(options) {
  ...
  // CLI --label / --milestone / --state flags seed the initial selection
  const seedFilters = Object.assign({}, persisted);
  if (initialFilter.label) {
    seedFilters.activeLabels = [initialFilter.label];
  }
  if (initialFilter.milestone) {
    seedFilters.activeMilestones = [initialFilter.milestone];
  }
  if (initialFilter.state) {
    seedFilters.activeState = initialFilter.state;
  }
  ...
  /**
   * Apply filterState then fuzzy search, returning the display list.
   * @returns {Object[]}
   */
  function _applyFiltersAndSearch() {
    const afterFilter = filterState.apply(issues);
    return query ? _searchIn(afterFilter, query) : afterFilter;
  }

  // FuzzySearch doesn't have a searchIn method — we need a helper that creates
  // a temporary FuzzySearch over a subset.
  function _searchIn(subset, q) {
    if (!q) return subset;
    const localSearch = new FuzzySearch(subset, { keys: ['title', 'number', 'labels'] });
    return localSearch.search(q);
  }
  ...
  keyboard.on('scroll-down', () => {
    if (focusPane === 'filter') return; // handled separately
    selectedIndex++;
    clampIndex();
    draw();
  });
  ...
  keyboard.on('tab-focus-reverse', () => {
    const order = ['list', 'detail', 'filter'];
    const current = order.indexOf(focusPane);
    focusPane = order[(current - 1 + order.length) % order.length];
    draw();
  });
  ...
  keyboard.on('filter-focus', () => {
    if (focusPane === 'filter') {
      // Toggle: pressing f again closes the pane
      _savePersistedFilters(filterState);
      focusPane = 'list';
    } else {
      focusPane = 'filter';
